...
TOFU_TABLES_SQL = [
    ...
    """,
    """
    CREATE TABLE IF NOT EXISTS tofu_rotation_history (
...
def tofu_ensure_tables(db_conn):
    """Ensure TOFU tables exist."""
    for sql in TOFU_TABLES_SQL:
        db_conn.execute(sql)
    db_conn.commit()
...
# TOFU Key Management: Ensure tables exist at startup
db_conn = sqlite3.connect(DB_PATH)
tofu_ensure_tables(db_conn)
db_conn.close()
